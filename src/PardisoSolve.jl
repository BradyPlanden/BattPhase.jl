...
@inline function pdrk3asolve(Y,Φ,Φₜ,Φₘ,F,dt,N,M,δ,ki,ymid,j,Ntot,tt,tf,TT,V,V1,V2,ff,dᵦ,ν,vv,h,Φₐ,Ydata,Δₜ)
    i = 1
    ...
    while tt < tf
        ...
        Upwind!(ymid,F,N,M,δ,h)
        Ef2!(Y,ymid,Φ,F,Δₜ,ν,ki,N,M)

        #Ef3!
        Upwind!(ymid,F,N,M,δ,h)
        Ef3!(Y,ymid,Φₘ,F,Δₜ,ν,ki,N,M)
