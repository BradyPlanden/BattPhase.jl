using BattPhase, LinearAlgebra, Plots, BenchmarkTools, SparseArrays, ProfileView, Infiltrator, DifferentialEquations#, MKL
    ...
    function BattEqs!(Y₀,y,δ,ki₀,ff,N,M,h)
    ...
    tspan = tf-tt
    prob = ODEProblem(BattEqs!,(Y₀,Φ₀,δ,ki₀,ff,N,M,h),tspan)
...
# B=rand(26244,26244);b=rand(26244);
# B = sparse(B)
# @inline @views function luQ(B,b)
#      Q=lu(B)
#      k = Q\-b
# end

# @inline @views function backslash(B,b) 
#     k = B\-b
# end

# @btime luQ(j₀,ff)
# @btime backslash(j₀,ff)